...
- [ ] 比對 canonical vs workspace：`diff ~/.hermes/skills/neat-freak/SKILL.md ~/Documents/Agent/skills/neat-freak/SKILL.md`
  - ⚠️ **先 diff 再檢查 frontmatter**：`head` 在複雜 TTY 環境下可能顯示誤導內容（2026-06-26）。`diff` 結果才是權威；`head` 有異議時用 `read_file` 或 Python 確認。
- [ ] 如有 drift 且以 canonical 為準 → `cp ~/.hermes/skills/neat-freak/SKILL.md ~/Documents/Agent/skills/neat-freak/SKILL.md`，然後 `git -C ~/Documents/Agent add -A && git commit`（`diff` 報告 identical 時仍需 `add -A` 重新 stage）
