...
description: >
  ...
  FR: "trier la boite de rÃ©ception", "vider l'inbox", "classer mes notes".
...
# Inbox Triage â€” Intelligent Inbox Processing & Filing
...
åœ¨æ‰¹æ¬¡è™•ç† `00-Inbox/` å‰ï¼Œå„ªå…ˆä½¿ç”¨ vault index å…ˆç¸®å°å€™é¸ï¼Œè€Œä¸æ˜¯ç›´æŽ¥å°æ•´å€‹ vault åšé‡æŽƒã€‚
...
- `by-classification`
- å¿…è¦æ™‚ç”¨ `related-notes`

ç”¨é€”ï¼š

- åˆ¤æ–·æ˜¯å¦å…¶å¯¦å·²æœ‰æ­£å¼ç‰ˆæœ¬
- æ‰¾æœ€å¯èƒ½çš„åˆ†é¡žè·¯å¾‘èˆ‡æ—¢æœ‰ä¸»é¡Œç¾¤
- æ‰¾æ‡‰è©²äº’é€£çš„ç›¸é„°ç­†è¨˜
...
- æ‰¾åŒä¸»é¡Œç­†è¨˜ç¾¤
- ç¢ºèªé€™ç¯‡è‹¥ç§»å‹•å¾Œæ˜¯å¦èƒ½è¢«æ—¢æœ‰ MOC / ä¸»é¡Œé ç™¼ç¾
- é™ä½Žé‡è¤‡å»ºæª”èˆ‡éŒ¯èª¤æ­¸æª”
...
- `notes.db` æš«æ™‚ä¸å¯ç”¨
- index æŸ¥è©¢çµæžœä¸è¶³ä»¥åˆ¤æ–·å®‰å…¨ç›®çš„åœ°
...
During triage, if you encounter a situation you can't fully resolve â€” **don't ask the user, and don't skip silently**. Signal the dispatcher via your output.

### When to suggest another agent

- **Architect** â€” **MANDATORY.** Before filing any note, classify the missing-destination case correctly:
...
- **Librarian** â€” when you find duplicates, broken links, or frontmatter issues that go beyond this triage session
- **Connector** â€” when you file a batch of notes that seem highly interconnected and should be cross-linked
- **Seeker** â€” when you need to verify if a similar note already exists before creating wikilinks

Always include your proposed solution and what you did in the meantime. Then **continue with the rest of the triage** â€” don't block.
...
1. [Meeting] 2026-03-18 â€” Sprint Planning Q2
2. [Idea] 2026-03-19 â€” New Onboarding Approach
3. [Task] 2026-03-20 â€” Call Supplier
...
```

### Step 2: Classify & Route

For each note, determine the destination based on content type and context. **Analyze the full content, not just the frontmatter** â€” auto-detect project and area from the text body, mentioned people, topics, and keywords:
...
| Unclear | Keep in Inbox, mark `Needs Review` | Ambiguous or unsafe â€” record reason and continue |

### Step 3: Pre-Move Checklist (for each note)

Before moving any note:

1. **Verify destination exists** â€” if the parent structure already exists and only an obvious low-risk subfolder is missing, create it; otherwise leave the note in `00-Inbox/` and escalate to Architect
2. **Check for duplicates** â€” search the destination for notes with similar titles or content
...
5. **Extract action items** â€” if the note contains tasks, ensure they're also captured in the relevant Daily Note or project note
...
## Notes
- [[Note Title 1]] â€” {{one-line summary}}
- [[Note Title 2]] â€” {{one-line summary}}
...
- [[02-Areas/Marketing/Old Campaign Brief]] â€” last updated 2026-02-10
- [[01-Projects/Beta/Initial Scope]] â€” last updated 2026-01-28

Needs Review (left in Inbox):
- "random notes" â€” ambiguous destination; safe routing not established
...
4. Don't auto-archive â€” always get confirmation
...
- **People mentioned** â€” which projects are they associated with?
- **Temporal context** â€” when was this written and what was the user working on at that time?
- **Wellness content** â€” notes related to wellness go to Health area (if configured)
- **Technical content** â€” notes with code or architecture discussions go to the relevant project
...
1. Never delete notes â€” only move them
2. Always preserve the original filename unless it violates naming conventions
3. Rename files to match convention: `YYYY-MM-DD â€” {{Type}} â€” {{Title}}.md`
...
- Respect existing tag taxonomy â€” don't invent new tags without checking `Meta/tag-taxonomy.md`
...
Read `Meta/states/sorter.md` if it exists. It contains notes you left for yourself last time â€” e.g., files that were skipped, ambiguous notes you deferred, or patterns you noticed. If the file does not exist, this is your first run â€” proceed without prior context.
...
[Your notes here â€” max 30 lines]
```
